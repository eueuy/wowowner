(*
  ...
  TMPHexEditorEx v 04-18-2004<br>
  ...
  published
    ...
    *)
    ...
    (* printing flags:<br><br>
      ...
      - pfSelectionBold: render the current selection using a bold font (if pfSelectionOnly isn't set)<br>
...
function GetSomeData(const PData: Pointer; const HGlobal: THandle; Format:
  TClipFormat; const DataSize: integer; const UnicodeBigEndian: Boolean): string;
var
        ...
        begin
          ...
          case LWrdCurrent of
            ...
            SetString(LStrFormatName, LszBuffer,
              GetClipboardFormatName(LWrdCurrent, LszBuffer, sizeof(LszBuffer)));
...
begin
  ...
  Result := (not (ReadOnlyView (*or NoSizeChange*))) and
    Succeeded(OLEGetClipboard(LifData)) and (SupportsOLEData(LifData, 0, Point(0,
...
function TMPHexEditorEx.InsertOLEData(const dataObj: IDataObject; const
  grfKeyState: longint; const pt: TPoint; var dwEffect: longint; const Operation:
  TMPHOLEOperation): HRESULT;
var
  LRecStg: TStgMedium;
                  ...
                  begin
                    ...
                    begin
                      if (SelCount = 0) or (not IsSelected(LIntPos)) then
                        Replace(@LStrData[1], LIntPos, LIntData, LIntData, UNDO_DROPPED)
                      else
                        ReplaceSelection(@LStrData[1], LIntData, UNDO_DROPPED)
                    end;
                  end;
...
function TMPHexEditorEx.SupportsOLEData(const dataObj: IDataObject; const
  grfKeyState: integer; const pt: TPoint; var dwEffect: integer; const Operation:
  TMPHOLEOperation): HRESULT;
begin
  Result := S_FALSE;
...
begin
  ...
  begin
    if Bool(grfKeyState and MK_SHIFT) then
      dwEffect := DROPEFFECT_MOVE
    else
      dwEffect := DROPEFFECT_COPY;
    ...
    with BookMark[LIntLoop] do
      if mPosition <> -1 then
        Result := Result + IntToRadixLen(LIntLoop, 16, 2) +
          IntToRadixLen(mPosition, 16, 16) + IntToRadixLen(integer(mInCharField),
          16, 2);
  if Result <> '' then
  begin
    LIntCheck := 0;
...
{$IFDEF DELPHI6UP}
const
  PUBLIC_PROPS: array[0..64] of string = ('ShowRuler',
    'DrawGutter3D',
    'CreateBackup',
    'BackupExtension',
    'OleDragDrop',
...
begin
  if (UndoStorage.UpdateCount = 0) and (FCreateUndoOnUndoUpdate or (StrUndoDesc <> '')) then
  begin
    ...
    begin
      ...
      case S[LIntLoop] of
        ...
        '>':
          begin
            if not FEditor.UnicodeChars then
              Result := Result + MPHTranslationDesc[FEditor.Translation]
            else
            begin
        ...
        '<':
          begin
            if not FEditor.UnicodeChars then
              Result := Result + MPHTranslationDescShort[FEditor.Translation]
            else
            begin
              if not FEditor.UnicodeBigEndian then
...
// return one line of data
  ...
  begin
    ...
    if (FEditor.Row - FEditor.FixedRows) = (CurPosition div FEditor.BytesPerRow)
      then
      Result.Attributes := StringOfChar(#16, Length(LStrPart))
    else
      Result.Attributes := StringOfChar(#0, Length(LStrPart));
    ...
    begin
      ...
      if FEditor.IsSelected(CurPosition - 1 + LIntLoop) then
        Result.Attributes := Result.Attributes + StringOfChar(char(Ord(LChrAttr)
          or 16), Length(LStrPart))
      else
        Result.Attributes := Result.Attributes + StringOfChar(LChrAttr,
          Length(LStrPart));

      if LIntLoop < FEditor.BytesPerRow then
      begin

        if (FEditor.BytesPerBlock > 1) and ((LIntLoop mod FEditor.BytesPerBlock) = 0) then
        begin
          Result.Text := Result.Text + ' ';
          Result.Attributes := Result.Attributes + LChrAttr;
        end;
        ...
        begin
          Result.Text := Result.Text + ' ';
          Result.Attributes := Result.Attributes + LChrAttr;
          LChrAttr := char(3 - Ord(LChrAttr));
        end;
      end;
    end;

    Result.Text := Result.Text + '  ';
    Result.Attributes := Result.Attributes + LChrAttr+LChrAttr;
    ...
    begin
      ...
      begin
        if (CurPosition + LIntLoop - 1) > EndPosition then
          ...
          Result.Text := Result.Text +
            FEditor.TranslateToAnsiChar(FEditor.Data[CurPosition + LIntLoop - 1]);
        if FEditor.IsSelected(CurPosition - 1 + LIntLoop) then
          Result.Attributes := Result.Attributes + #19
        else
          Result.Attributes := Result.Attributes + #3;

        if (FEditor.BytesPerBlock > 1) and FEditor.SeparateBlocksInCharField and
          ((LIntLoop mod FEditor.BytesPerBlock) = 0) then
        begin
          Result.Text := Result.Text + ' ';
          Result.Attributes := Result.Attributes + #4;
        end;
      ...
      begin
        ...
        begin
          ...
          if LWChrText < #32 then
            LWChrText := WideChar(FEditor.MaskChar);
          Result.Text := Result.Text + LWChrText;
          if FEditor.IsSelected(CurPosition + (LIntLoop * 2)) then
            Result.Attributes := Result.Attributes + #19
          else
            Result.Attributes := Result.Attributes + #3;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        TextOut(FCanvas.Handle, LeftPos, Y, PChar(LStrLeft), Length(LStrLeft));
      end;
      if LStrCenter <> '' then
      begin
...
begin
  ...
  try
    if (pfMonochrome in FFlags) or (not (pfUseBackgroundColor in FFlags)) then
    ...
    while (LIntWidth > (FMargins.Right - FMargins.Left)) and (FCanvas.Font.Size
      > 1) do
    begin
    ...
    LIntHeight := Round(FCanvas.TextHeight(LRecTextAttr.Text) * 1.2);
    if LIntHeight = FCanvas.TextHeight(LRecTextAttr.Text) then
      inc(LIntHeight);
      ...
      begin
        ...
        begin
          if not (pfMonochrome in FFlags) then
            case (Ord(LRecTextAttr.Attributes[LIntLoop]) and 15) of
              0: FCanvas.Font.Color := FEditor.Colors.Offset;
              1: FCanvas.Font.Color := FEditor.Colors.EvenColumn;
              2: FCanvas.Font.Color := FEditor.Colors.OddColumn;
            else
              FCanvas.Font.Color := FEditor.Font.Color;
            end;
          ...
          if FFlags * [pfSelectionBold, pfSelectionOnly] = [pfSelectionBold]
            then
          begin
            if (Ord(LRecTextAttr.Attributes[LIntLoop]) and 16) = 16 then
              FCanvas.Font.Style := [fsBold]
            else
              FCanvas.Font.Style := [];
          end;
          TextOutW(FCanvas.Handle, LIntLeft, LIntY,
            @LRecTextAttr.Text[LIntLoop], 1);
          Inc(LIntLeft, FCanvas.TextWidth('w'));
        end;
        FCanvas.Font.Color := LclrSave;
    ...
    if FPrintHeaders[1] <> '' then
      if not JustCalc then
      begin
        DrawHeader(FMargins.Left, FMargins.Bottom - LIntHeight, FMargins.Right,
          FPrintHeaders[1]);
...
// count number of lines per page (as well number of pages)
